...
int main(int argc,char *argv[]){
    ...
    j=0;
    snprintf(file,100,"data/%s/SEP_with_a_defect_correlation_current_rho%d.txt",argv[1],a);
    fp=fopen(file,"r");
    fscanf(fp,"%lf %lf %lf\n",&A,&B,&g1);
    fscanf(fp,"%lf %lf %lf\n",&A,&B,&g2);
    fclose(fp);
    ...
    snprintf(file,100,"data/%s/SEP_current_fluctuation_theory2%d.txt",argv[1],a);
    fp=fopen(file,"w");
    for(i=1;i<L;++i){
