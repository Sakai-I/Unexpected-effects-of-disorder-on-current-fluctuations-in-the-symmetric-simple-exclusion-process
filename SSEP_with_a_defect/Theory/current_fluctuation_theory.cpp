...
#define taumax 10 //waiting time at the defect site
...
int main(){
    ...
    snprintf(file,100,"%d/current_rho/SEP_current_rho%d_2.txt",L,taumax);
    fp=fopen(file,"r");
    fscanf(fp,"%lf %lf %lf\n",&g1,&g1,&g1);
    fclose(fp);

    snprintf(file,100,"%d/current_rho/SEP_current_rho%d_%d.txt",L,taumax,L-2);
    fp=fopen(file,"r");
    fscanf(fp,"%lf %lf %lf\n",&g2,&g2,&g2);
    fclose(fp);
    ...
    snprintf(file,100,"%d/current_theory/SEP_current%d",L,taumax);
    fp=fopen(file,"w");
    for(i=1;i<L;++i){
